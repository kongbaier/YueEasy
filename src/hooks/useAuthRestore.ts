...
import { ncm, getNcmCookie, clearNcmCookie } from "@/services/ncm";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("[useAuthRestore] cookie from Rust:", cookie ? `${cookie.substring(0, 50)}...` : "(empty)");
        if (!cookie) return;

        const res = await ncm.loginStatus();
        console.log("[useAuthRestore] loginStatus response:", JSON.stringify(res));
        ...
        const profile =
          res.data?.profile ??
          (res as Record<string, unknown>).profile as
            | { userId: number; nickname: string; avatarUrl: string }
            | undefined;

        const statusCode =
          res.data?.code ?? (res as Record<string, unknown>).code;

        console.log("[useAuthRestore] resolved statusCode:", statusCode, "profile:", profile);
