...
listen<PlayerEvent>('player:queue-changed', (e) =>
  usePlayerMirror.setState({ queue: e.payload.items, currentIndex: e.payload.current_index })
);
// ...
```
...
export function usePlayer() {
  ...
  return useMemo(() => ({ currentTrack, playing, playNext }), [currentTrack, playing, playNext]);
}
```
